...
const ReportsPage: React.FC = () => {
  ...
  const rows = history.map((entry, index) => ({
    id: index,
    timestamp: formatTimestamp(entry.timestamp), // ✅ Formatted timestamp
    cpuLoad: entry.cpu?.currentLoad ? entry.cpu.currentLoad.toFixed(2) : 'N/A',
    memoryUsed: entry.memory?.used || 'N/A',
    diskUsage: entry.disk?.[0]?.used ? ((entry.disk[0].used / entry.disk[0].size) * 100).toFixed(2) : 'N/A',
    networkSpeed: entry.network?.[0]?.rx_bytes ? (entry.network[0].rx_bytes / 1024 / 1024).toFixed(2) : 'N/A'
  }));
  ...
  const exportPDF = () => {
    ...
    const tableData = rows.map(row => [
      formatTimestamp(row.timestamp), // ✅ Formatted timestamp
      row.cpuLoad || 'N/A',
      row.memoryUsed || 'N/A',
      row.diskUsage || 'N/A',
    ...
    // ✅ Corrected way to use autoTable
    autoTable(doc, {
      head: [['Timestamp', 'CPU Load (%)', 'Memory Used (MB)', 'Disk Used (%)', 'Network Speed (MB/s)']],
      body: tableData,
      startY: 20,
    ...
    <Container maxWidth="lg" sx={{ mt: 4 }}>
      ...
      <Paper elevation={3} sx={{ p: 3, mt: 3}}>
        ...
        <Typography variant="body1">
          This page displays **real-time system performance data** in a structured table.
          The table updates automatically every **5 seconds** with the latest system stats, including:
        </Typography>
        <Typography component="ul" sx={{ mt: 1, pl: 2 }}>
          ...
          <Typography component="li">**Disk Usage (%)** – Percentage of disk space used.</Typography>
          <Typography component="li">**Network Speed (MB/s)** – Download speed.</Typography>
        </Typography>
        <Typography variant="body1" sx={{ mt: 1 }}>
          You can **export** this data as a **CSV file** or **download a PDF report** using the buttons below.
        </Typography>
      </Paper>
